...
class dll_timer {
public:
    dll_timer() : prev(NULL), next(NULL){}

public:
   time_t expire;   // 任务超时时间，这里使用绝对时间
   void (*cb_func)( http_conn* ); // 任务回调函数，回调函数处理的客户数据，由定时器的执行者传递给回调函数
   http_conn* user_data;
   dll_timer* prev;    // 指向前一个定时器
   dll_timer* next;    // 指向后一个定时器
};

// 定时器链表，它是一个升序、双向链表，且带有头节点和尾节点。
class sort_timer_dll {
public:
    sort_timer_dll();
    // 链表被销毁时，删除其中所有的定时器
    ~sort_timer_dll();
    ...
    // 将目标定时器timer添加到链表中
    void add_timer( dll_timer* timer );
    
    /* 当某个定时任务发生变化时，调整对应的定时器在链表中的位置。这个函数只考虑被调整的定时器的
    超时时间延长的情况，即该定时器需要往链表的尾部移动。*/
    void adjust_timer(dll_timer* timer);

    // 将目标定时器 timer 从链表中删除
    void del_timer( dll_timer* timer );
...
private:
    ...
    void add_timer(dll_timer* timer, dll_timer* lst_head);
