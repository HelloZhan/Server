...
sort_timer_dll::sort_timer_dll():head( NULL ), tail( NULL ){}

// 链表被销毁时，删除其中所有的定时器
sort_timer_dll::~sort_timer_dll() {
    dll_timer* tmp = head;
    while( tmp ) {
        head = tmp->next;
        delete tmp;
...
// 将目标定时器timer添加到链表中
void sort_timer_dll::add_timer( dll_timer* timer ) {
...
void sort_timer_dll::adjust_timer(dll_timer* timer)
{
    if( !timer )  {
        return;
    }
    dll_timer* tmp = timer->next;
...
// 将目标定时器 timer 从链表中删除
void sort_timer_dll::del_timer( dll_timer* timer )
{
    if( !timer ) {
        return;
    }
...
void sort_timer_dll::add_timer(dll_timer* timer, dll_timer* lst_head)  {
    dll_timer* prev = lst_head;
    dll_timer* tmp = prev->next;
